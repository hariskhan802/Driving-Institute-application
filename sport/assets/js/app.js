...
jQuery(document).ready(function() {
    ...
    var apiURL = "//localhost/msa_portal/admin/api/";
    var toaster = function(type, msg){
        console.log(type);
        console.log(msg);
        $.toast({
            text    : msg,
    ...
    appendDropdownDefaultOption('program_drp', 'Program');
    appendDropdownDefaultOption('level_drp', 'Level');
    function getProgramsByClubDropdown(club_id = null){
        if(club_id == null) return;
        let program = [];
        if(global_programs.length != 0){
            $.each(global_programs, function(index, value){
                if(value.club_id == club_id) program.push(value);
            });
        }
        if(program != []){
            $.each(program, function(index, value){
                $("#program_drp") .append($("<option></option>")
                    .attr("value", value.id)
                    .text(value.program_name));
            });
        }
        else appendDropdownNullOption('program_drp', 'Programs');
    }
    $('#club_drp').on('change', function (){
        emptyDropdown('program_drp');
        emptyDropdown('level_drp');
        getProgramsByClubDropdown(this.value);
    });
    
    function getLevelsByProgramDropdown(program_id = null){
        if(program_id == null) return;
        let level = [];
        if(global_levels.length != 0){
            $.each(global_levels, function(index, value){
                if(value.program_id == program_id) level.push(value);
            });
        }
        if(level != []){
            $.each(level, function(index, value){
                $("#level_drp") .append($("<option></option>")
                    .attr("value", value.id)
                    .text(value.level_name));
            });
        }
        else appendDropdownNullOption('level_drp', 'Levels');
    }
    $('#program_drp').on('change', function (){
        emptyDropdown('level_drp');
        getLevelsByProgramDropdown(this.value);
    });
    function appendDropdownDefaultOption(id, label){
        if(id == 'undefined') return;
        if(label == 'undefined') return;
        $('#' + id).append($("<option></option>").attr("value", '').text('Select ' + label));
    }
    function emptyDropdown(id = null){
        if(id == null) return;
        $('#' + id).children('option:not(:first)').remove();
    }
    function appendDropdownNullOption(id, label){
        if(id == 'undefined') return;
        if(label == 'undefined') return;
        $('#' + id).append($("<option></option>").attr("value", '').attr("disabled", '').text('No ' + label));
    }
    window.returnSelectedTerm = function(term_id = null){
        if(term_id == null) return;
        let term = {};
        if(global_terms.length != 0){
            let flag = false;
            $.each(global_terms, function(index, value){
                if(!flag){
                    if(value.id == term_id){
                        term = value;
                        flag = true;
    ...
    window.returnSelectedLevel = function(level_id = null){
